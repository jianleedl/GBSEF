...
from tools import str2seq, strs2seq
    ...
    def check_diff(self, sentence, perturbed_sentence):
        ...
        for i in range(len(words)):
            if words[i] != perturbed_words[i]:
                diff.append((i, words[i], perturbed_words[i], None, 'tagNONE'))
        return diff
...
class GAAdversary(Adversary):
    ...
    def check_return(self, perturbed_words, ori_words, ori_label):
        ...
        change_tuple_list = self.check_diff(clean_text, perturbed_text)
        sub_rate = len(change_tuple_list) / len(ori_words)
        ne_rate = 0.0
        if sub_rate > self.max_perturbed_percent:
            return False, perturbed_text, ori_label, sub_rate, ne_rate, change_tuple_list
        else:
            ...
            return adv_label != ori_label, perturbed_text, adv_label, sub_rate, ne_rate, change_tuple_list
    ...
    def run(self, sentence, ori_label):
        ...
        if np.sum(w_select_probs) == 0:
            return False, sentence, ori_label, 0, 0, []
