...
from Attacker import adversarial_paraphrase, textfool_perturb_text, GAAdversary
...
from tools import logging, str2seq, get_time, make_dir_if_not_exist, \
    write_standard_data, read_standard_data

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', help='choose the dataset', choices=config_dataset_list, default="IMDB")
parser.add_argument('--model', help='choose the target model',
                    choices=config_model_lists, default="LSTM_enhanced")
parser.add_argument('--attack', help='choose a attack algorithm', choices=config_attack_list, default="PWWS")
...
class Fooler():
    ...
    def get_fool_sentence_ga(self, sentence:str, label:int, index:int):
        start = time.perf_counter()
        maxlen = config_data[config_dataset].padding_maxlen
        vector = str2seq(sentence, self.vocab, self.tokenizer, maxlen).to(config_device)
        ...
        if predict == label:
            success, sentence, _, sub_rate, NE_rate, change_tuple_list = GAAdversary(
                self.net, self.vocab, self.tokenizer, maxlen).run(sentence, label)
            if success:
                flag = 1
            else:
                flag = 0
