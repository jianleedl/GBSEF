...
def strs2seq(sentences:[str], vocab, tokenizer, maxlen:int)->torch.Tensor:
    X = []
...
def write_data(dataset):
    train_path = config_data[dataset].train_data_path
    test_path = config_data[dataset].test_data_path
    if dataset == "IMDB":
        data_path = r'./dataset/IMDB/aclImdb/train'
        data_train, label_train = read_IMDB_origin_data(data_path)
        data_path = r'./dataset/IMDB/aclImdb/test'
        data_test, label_test = read_IMDB_origin_data(data_path)
    elif dataset == "AGNEWS":
        data_path = r'./dataset/AGNEWS/ag_news_csv/train.csv'
        data_train, label_train = read_AGNEWS_origin_data(data_path)
        data_path = r'./dataset/AGNEWS/ag_news_csv/test.csv'
        data_test, label_test = read_AGNEWS_origin_data(data_path)
    elif dataset == "YAHOO":
        data_path = r'./dataset/YAHOO/yahoo_answers_csv/train.csv'
        data_train, label_train = read_YAHOO_CSV(data_path)
        data_path = r'./dataset/YAHOO/yahoo_answers_csv/test.csv'
        data_test, label_test = read_YAHOO_CSV(data_path)
    write_standard_data(data_train, label_train, train_path)
    write_standard_data(data_test, label_test, test_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', choices=config_dataset_list, default='IMDB')
    ...
    # write_data(dataset)
    test_data = MyDataset(dataset, None, is_train=False, data_path=config_data[dataset].test_data_path, is_to_tokens=False)
