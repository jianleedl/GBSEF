...
class SynonymGenerator():
    ...
    def read_syn_csv(self, path, syn_dict=None):
        if not syn_dict: syn_dict = self.syn_dict
    ...
    def write_syn_csv(self, path, syn_dict=None):
        if not syn_dict: syn_dict = self.syn_dict
    ...
    def build_syn_dict(self, vocab:'Vocab', path):
        ...
        for key, value in tqdm(vocab.word_dict.items()):
            if value == 0 and key in self.syn_dict: continue
            res = self.get_similarity_words(key)
            self.syn_dict[key] = res
            q = deque([res])
            while q:
                words = q.popleft()
                for word in words:
                    if word not in self.syn_dict:
                        if key not in syn_dict_label: syn_dict_label[key] = set()
                        syn_dict_label[key].add(word)
                        self.syn_count += 1
                        res = self.get_similarity_words(word)
                        self.syn_dict[word] = res
                        q.append(res)
        ...
        self.write_syn_csv(path[:-4] + "_label.csv", syn_dict_label)
